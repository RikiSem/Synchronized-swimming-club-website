...
function ShowGalery(){
    ...
    $countPage = ceil($numRow / 8);

    $numPage = $_SERVER['QUERY_STRING'];

    
    echo("<div class='numPagesDiv'>");
    for($a = 1; $a != $countPage; $a++){
        echo("<a class='numPages' href='galery.php?{$a}'>{$a}</a>");
    }
    echo("</div>
    <div class='FotoGaleryDiv'>");
    $MinID = $numPage * 8 - 7;
    $MaxID = $numPage * 8+1;
    $zapros = "SELECT * FROM `galery` WHERE `id`>= '$MinID' AND `id`< '$MaxID'";
    $result = mysqli_query($link,$zapros);
    $numRow = mysqli_num_rows($result);
    for($i = 0; $i != $numRow; $i++){
        $text = mysqli_fetch_assoc($result);
        echo("<div class='FotoGalery'>
        <a href = '../imgsFromDatabase/galery/{$text['fotoLink']}'><img width='300em' height='500em' src='../imgsFromDatabase/galery/{$text['fotoLink']}' alt='Фото'></a>
    </div>");
    }
}
?>